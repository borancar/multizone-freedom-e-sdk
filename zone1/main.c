...
// ------------------------------------------------------------------------
	...
	for (int i=0, first=1; i<COUNT; i++){
		...
		asm volatile ( // 2 = ECALL_YIELD(), 6 = ECALL_CSRR_MCYCLE() - timeout 1s (6.5Mcycles/~100cycles)

			"   li %1, 150; li %2, 650000;"
		...
		cycles[i] 	   = (int)r0; // 1 loop = 47 cycles (RV64)
	...
	if (cycles[0]==0){
		...
		if (ctxsw_instr[0]>0){
			printf("\n");
			min = ctxsw_instr[0], med = ctxsw_instr[COUNT/2], max = ctxsw_instr[COUNT-1];
...
// ------------------------------------------------------------------------
	...
	printf("======================================================================\n");
	printf("      	       Hex Five MultiZone(TM) Security v.0.1.0                \n");
...
/*			uint32_t r1, r2, r3, r4;
			asm ("li a0, 6; ecall; mv %0, a1; mv %1, a0;" // ECALL_CSRR_MCYCLE()
				 ...
				 "li a0, 6; ecall; mv %2, a1; mv %3, a0;" // ECALL_CSRR_MCYCLE()
